...
int main(int argc, char** argv)
{
    ...
    if (argc != 6) {
        fprintf(stderr,
                "Usage: bake-copy-from <server addr> <mplex id> <identifier "
                "file> <output file> <size>\n");
        fprintf(stderr,
                "  Example: ./bake-copy-from tcp://localhost:1234 3 "
                "/tmp/bb-copy-rid.0GjOlu /tmp/output.dat 256\n");
        return (-1);
    }
    svr_addr_str = argv[1];
    ...
    /* transfer data */
    uint64_t bytes_read;
    ret = bake_read(bph, tid, rid, 0, local_region, size, &bytes_read);
    if (ret != 0) {
