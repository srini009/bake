...
{
    fprintf(stderr,
            "Usage: proxy-server-daemon [OPTIONS] <listen_addr> "
            "<bake_server_addr> <bake mplex id>\n");
    fprintf(stderr, "       listen_addr is the Mercury address to listen on\n");
    fprintf(
        stderr,
        "       bake_server_addr is the Mercury address of the BAKE server\n");
    fprintf(stderr,
            "       [-b] to batch the BAKE region create, write, and persist "
            "operations in one RPC\n");
    fprintf(
        stderr,
...
int main(int argc, char** argv)
{
    ...
    ret = bake_provider_handle_create(g_proxy_svr_ctx->bcl, bake_svr_addr,
                                      opts.bake_mplex_id,
    ...
    ret = bake_probe(g_proxy_svr_ctx->svr_bph, 1, &g_proxy_svr_ctx->svr_bti,
                     &num_targets);
    if (ret < 0) {
...
static void proxy_write_ult(hg_handle_t handle)
{
    ...
    if (g_proxy_svr_ctx->batch_rpc) {
        ...
        ret = bake_create_write_persist_proxy(
            g_proxy_svr_ctx->svr_bph, g_proxy_svr_ctx->svr_bti, in.bulk_handle,
            in.bulk_offset, in.bulk_addr, in.bulk_size,
            &(g_proxy_svr_ctx->the_rid));
        assert(ret == 0);
    } else {
        /* create BAKE region to store this write in */
        ret = bake_create(g_proxy_svr_ctx->svr_bph, g_proxy_svr_ctx->svr_bti,
                          in.bulk_size, &(g_proxy_svr_ctx->the_rid));
        assert(ret == 0);

        /* perform proxy write on behalf of client */
        ret = bake_proxy_write(g_proxy_svr_ctx->svr_bph,
                               g_proxy_svr_ctx->svr_bti,
                               g_proxy_svr_ctx->the_rid, 0, in.bulk_handle,
                               in.bulk_offset, in.bulk_addr, in.bulk_size);
        assert(ret == 0);

        /* persist the BAKE region */
        ret = bake_persist(g_proxy_svr_ctx->svr_bph, g_proxy_svr_ctx->svr_bti,
                           g_proxy_svr_ctx->the_rid, 0, in.bulk_size);
        assert(ret == 0);
    }
...
static void proxy_read_ult(hg_handle_t handle)
{
    ...
    /* perform proxy write on behalf of client */
    uint64_t bytes_read;
    ret = bake_proxy_read(g_proxy_svr_ctx->svr_bph, g_proxy_svr_ctx->svr_bti,
                          g_proxy_svr_ctx->the_rid, 0, in.bulk_handle,
                          in.bulk_offset, in.bulk_addr, in.bulk_size,
                          &bytes_read);
    assert(ret == 0);
